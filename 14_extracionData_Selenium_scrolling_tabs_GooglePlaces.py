...
# OBJETIVO:
#     - Extraer reseñas escritas por usuarios en Google Places.
...
# Función para obtener el Script de Scrolling dependiendo de cuantos scrollings ya he hecho
...
driver = webdriver.Chrome('./chromedriver/chromedriver.exe', chrome_options=opts)
driver.get(
...
for review in restaurantsReviews:
    sleep(1)  # Evitar baneos de nuestra IP
    ...
    try:
        ...
        # ACTUALIZACIÓN> YA NO ES NECESARIO POR QUE POR DEFECTO YA ESTAREMOS AQUÍ
        ...
        for userReview in userReviews:
            # Obtener la información de cada review
            ...
            userParsedRating = float(''.join(filter(str.isdigit or str.isspace,
                                                    reviewRating)))  # Codigo para solamente quedarme con los digitos de una cadena. En la clase nos quedamos con toda la cadena.
    ...
    # Si ocurre algún error, no me complico, cierro el tab abierto, y reinicio el contexto al tab original
    except Exception as e:
        print(e)
        driver.close()
        driver.switch_to.window(driver.window_handles[0])
